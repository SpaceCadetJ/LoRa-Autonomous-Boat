...
class Sheet:
    ...
    def render(self, root_uuid, root=False, sheets_block=''):
        ...
        out = ['(kicad_sch', '\t(version 20250114)', '\t(generator "gen_sch.py")', '\t(generator_version "9.0")',
               ...
               f'\t(title_block\n\t\t(title "{esc(self.title)} - {self.name}")\n\t\t(date "{"2026-09-17" if self.proj != PROJ else "2025-05-09"}")\n\t\t(rev "{"V2 draft" if self.proj != PROJ else "as fabricated (senior design v5.brd)"}")\n\t\t(company "University of Arkansas EECS")\n\t\t(comment 1 "{esc(self.descr)}")\n\t\t(comment 2 "{"Generated by hardware/kicad_v2/tools/gen_v2.py from v2_design.py; see docs/V2_REQUIREMENTS.md" if self.proj != PROJ else "Generated by hardware/kicad/tools/gen_sch.py from pstxnet.dat (2025-05-09 17:12); see README.md"}")\n\t)',
               '\t(lib_symbols\n' + libs + '\n\t)']
        out += self.items
