...
class Sheet:
    ...
    def _symbol(self, libid, x, y, rot, ref, value, props, sym, root_uuid, power=False, suuid=None):
        ...
        if self.name == 'Navigation' and ref in {'U6','U10','U11'}: top_clearance += 3
    ...
    def place(self, ref, sym, x, y, value, props, pin_nets, net_sheets, root_uuid):
        ...
        for (a,py,net), xs in vertical.items():
            dy = 1 if a == 90 else -1
            ey = py + dy * 5.08
            if net in POWER_NETS or self.name == "Navigation":
    ...
    def render(self, root_uuid, root=False, sheets_block=''):
        ...
        version = 'V1 / v5' if self.proj == PROJ else 'V2-IMU1'
        ...
        out = ['(kicad_sch','(version 20250114)','(generator "gen_sch.py")','(generator_version "9.0")',
               ...
               f'(title_block (title "{esc(title)}") (date "2026-09-18") (rev "{version}") (company "LoRa Controller Project") (comment 1 "IMU candidate: five U6 pin changes; not PCB integrated") (comment 2 "Build and review: docs/build/README.md"))',
               '(lib_symbols\n'+libs+'\n)']
        out += self.items
